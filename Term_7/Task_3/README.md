...
Рассмотрим вложенные циклы из программы в файле `./src/variant_3a.c`:
```cpp
for (i = 0; i < ISIZE; ++i) {
  for (j = 0; j < JSIZE; ++j)
    a[i][j] = sin(0.1 * a[i][j]);
...
Аналогично ситуации в эталонной программе для первого цикла получаем:

$$\boxed{D = (0,\ 0)}\ \ \  и\ \ \  \boxed{d = ("=",\ "=")}.$$

Получили `loop independent dependence`. Можно распараллелить первый цикл по `i` и `j`.

Рассмотрим теперь второй цикл, считая, что первый цикл уже точно отработал к началу работы второго цикла.

**1. Определим вектор расстояний:**

В каждой итерации обращение к массиву происходит к ячейке с индексом `(i, j)`:
1) Запись `S1`: `B[f(i, j)] = ...` с `f(i, j) = (i, j)`.
2) Чтение `S2`: `... = A[g(i, j)]` с `g(i, j) = (i+1, j)`.
...
$$(\lambda_i,\ \lambda_j) = (\kappa_i + 1,\ \kappa_j).$$
...
$$D = ((\kappa_i + 1) - \kappa_i,\ \kappa_j - \kappa_j) = (1,\ 0).$$

Итак:

$$\boxed{D = (1,\ 0)}\ \ \  и\ \ \  \boxed{d = ("<",\ "=")}.$$

<br>

**3. Тип зависимости и варианты распараллеливания:**

Поскольку первая компонента вектора направлений равна `"<"`, то зависимость является истинной (`true dependence`). Можем безопасно распараллелить по внутреннему циклу.

Воспользуемя следующей стратегией для `MPI`-распараллеливания:
1. Разделение строк массива между процессами.
2. Каждый процесс вычисляет свою часть массива `a` в первом цикле.
3. Для второго цикла требуется обмен граничными строками между соседними процессами.
4. Процесс `rank` отправляет свою последнюю строку процессу `rank - 1`.
5. Процесс `rank` получает строку от процесса `rank + 1` для вычисления граничных элементов.
...
Теперь мы можем сравнить последовательную и параллельную реализации:
```cpp
Sequential time: 0.237256 seconds

Parallel time: 0.071393 seconds    // Ускорили в 3.4 раз. 
```
<br>

Также построим график зависимости коэффициента ускорения от числа исполнителей для каждого из написанных вариантов программы:

![alt text](./results/variant_3a/speedup.png)