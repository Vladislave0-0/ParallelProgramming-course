...
В основе вычислений лежит формула площади Гаусса (также известная как «формула шнуровки»).
<br><br>
# Формула площади Гаусса 

Для многоугольника с вершинами $(x_0, y_0​), (x_1​, y_1​), \dotsc, (x_{n−1}​, y_{n−1}​)$, перечисленными в порядке обхода (по часовой или против часовой стрелки), площадь вычисляется как: 
$S = \dfrac{1}{2}\left|\sum\limits_0^{n-1}(x_iy_{i+1}-x_{i+1}y_i)  \right|$, 

где индекс $i+1$  берётся по модулю $n$ (то есть $x_n​=x_0$​, $y_n​=y_0$​ ).

Алгоритм имеет линейную сложность $O(n)$  и идеально подходит для распараллеливания, так как суммирование может быть выполнено по частям.
<br><br>
# Требования
